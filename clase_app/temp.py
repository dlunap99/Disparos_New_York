...
st.set_page_config(layout="wide") ## AMPLIAR 
st.markdown("<h1 style ='text-align: center; color: #1687CE;'>Histórico de disparos en Nueva York 🗽🔫 </h1>"
            , unsafe_allow_html =True) ##NOMBRE DE LA PAGINA,TITULO CENTRADO,COLOR DE LETRA
...
c4.markdown("<h3 style ='text-align: center;color: black;'>Top Barrios</h3>",unsafe_allow_html =True)
...
c1.markdown("<h3 style ='text-align: center;color: black;'>¿Donde han ocurrido disparos en NY?</h3>"
            ,unsafe_allow_html =True)
year = c1.slider("Año en que ocurrió el suceso",int(df["year"].min()),int(df["year"].max()))
c1.map(df[df['year']== year][["latitude","longitude"]])

c2.markdown("<h3 style ='text-align: center;color: black;'>¿A qué horas ocurren disparos en NY?</h3>"
            ,unsafe_allow_html =True)
hour = c2.slider("Hora en la ocurrió el suceso",int(df["hour"].min()),int(df["hour"].max()))
##df2=df[df["hour"] <= hour] acumulable las horas
df2= df[df["hour"]== hour]
...
c2.write(pdk.Deck( 
    ...
    radius = 100,
    extruded = True, #elevation
    elevation_scale = 4,
    elevation_range = [0,1000])]
    ))

##---------
st.markdown("<h3 style ='text-align: center;color: black;'>¿Cómo ha sido la evolución por barrio y año/mes en NY</h3>"
            ,unsafe_allow_html =True)
...
##### C4
c4.markdown("<h3 style ='text-align: center;color: black;'>¿Qué edad tienen los atacantes?</h3>"
            ,unsafe_allow_html =True)
...
######### C5
c5.markdown("<h3 style ='text-align: center;color: black;'>¿Qué edad tienen las victimas?</h3>"
            ,unsafe_allow_html =True)

df2 = df.groupby(['vic_age_group'])[['incident_key']].count().reset_index().rename(columns={'incident_key':'disparos'})

df2['vic'] = df2['vic_age_group'].replace({'940':'N/A','224':'N/A',
                                                       '1020':'N/A','UNKNOWN':'N/A'})
...
c6.markdown("<h3 style ='text-align: center;color: black;'>¿Cúal es el sexo del atacante?</h3>"
            ,unsafe_allow_html =True)
...
c7.markdown("<h3 style ='text-align: center;color: black;'>¿Cúal es el sexo de la victima?</h3>"
            ,unsafe_allow_html =True)
...
st.markdown("<h3 style ='text-align: center;color: black;'>Evolución de disparos por años en las horas con más y menos sucesos</h3>"
            ,unsafe_allow_html =True)
